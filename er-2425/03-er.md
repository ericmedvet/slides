...
.key[Perceptron]: $f\\subtext{P}: \\mathbb{R}^m \\to \\mathbb{R}$ with $\\vect{x} \\mapsto a(\\vect{w}^\\intercal\\vect{x}+b)=a(\\sum\\sub{i=1}^{i=m} w\\sub{i} x\\sub{i} +n)$
.cols[
.c70[
...
- often, $\\vect{w}$ is used to denote also the bias, hence $\\vect{w} \\in \\mathbb{R}^{m+1}$
]
.c30.center[
.diagram.center[
link([0,0,41,15],'a')
