...
.key[Perceptron]: $f\\subtext{P}: \\c{1}{\\mathbb{R}^m} \\to \\c{2}{\\mathbb{R}}$ with $\\c{1}{\\vect{x}} \\mapsto \\c{2}{a(\\vect{w}^\\intercal\\vect{x}+b)}=\\c{2}{a(\\sum\\sub{i=1}^{i=m} w\\sub{i} x\\sub{i} +n)}$
.cols[
.c80[
- $\\vect{w} \\in \\mathbb{R}$ is a vector of **synaptic weights**
...
.cols.compact[
.c20[
.w100p.center[![tanh function](images/plot-ann-act-tanh.png)]
]
.c30[
**Hyperbolic tangent** ($\\tanh$)
- image is $a(\\mathbb{R})=[-1,1]$
- useful to constraint the domain
]
.c20[
.w100p.center[![reLU function](images/plot-ann-act-relu.png)]
]
.c30[
...
- $i$-th neuron takes all outputs of $l-1$-th layer: $v\\sub{l,i}=f\\subtext{P}(\\vect{v}\\sub{l-1};\\vect{w}\\sub{l,i})$
  - short: $\\vect{v}\\sub{l}=f\\subtext{P}(\\vect{v}\\sub{l-1};\\vect{w}\\sub{l,1}) \\oplus \\dots \\oplus f\\subtext{P}(\\vect{v}\\sub{l-1};\\vect{w}\\sub{l,n\\sub{l}}) = \\varphi(\\vect{v}\\sub{l-1}; \\vect{w}\\sub{l,1} \\oplus \\dots \\oplus \\vect{w}\\sub{l,n\\sub{l}}) = \\varphi(\\vect{v}\\sub{l-1}; \\vect{w}\\sub{l})$
- **greatly** parallelizable! (GPUs 🤝🏼 NNs)
...
- hence $f\\subtext{MLP} = \\varphi(\\cdot;\\vect{w}\\sub{1}) \\circ \\dots \\circ \\varphi(\\cdot;\\vect{w}\\sub{m})$ has $\\theta \\in \\mathbb{R}^h$ params, with $\\vect{\\theta}=\\vect{w}\\sub{1} \\oplus \\dots \\oplus \\vect{w}\\sub{1}$
  - also known as **feedforward neural network**
- **params size** $h = n\\sub{2}(p+1)+n\\sub{3}(n\\sub{1}+1)+n\\sub{4}(n\\sub{3})+\\dots+n\\sub{m}(n\\sub{m-1}+1)$
]
...
Why are MLPs **relevant in general**?
- brief answer: because of the **universal approximation theorem**(s), stating that "feedforward networks with non-polynomial activation functions are dense in the space of continuous functions between two Euclidean spaces"
...
- more complex architectures
  ...
  - recurrent neural networks ($\\Rightarrow$ dynamical systems)
