...
- $f(\\vect{s})=(f\\sub{1}(\\vect{s}),f\\sub{2}(\\vect{s}))$, both to be minimized
  ...
  - trade-off betweeen objectives, i.e., you **cannot optimize both** at the same time with a single solution!
...
- NSGA-II with $n\\subtext{pop}=100$, $p\\subtext{xover}=0.8$
...
### In the search space ($\\mathbb{R}^2$)
