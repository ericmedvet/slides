...
.key[Perceptron]: $f\\subtext{P}: \\c{1}{\\mathbb{R}^p} \\to \\c{2}{\\mathbb{R}}$ with $\\c{1}{\\vect{x}} \\mapsto \\c{2}{a(\\vect{w}^\\intercal\\vect{x}+b)}=\\c{2}{a(\\sum\\sub{i=1}^{i=p} w\\sub{i} x\\sub{i} +b)}$
.cols[
.c80[
- $\\vect{w} \\in \\mathbb{R}^p$ is a vector of **synaptic weights**
...
- often, $\\vect{w}$ is used to denote also the bias, hence $\\vect{w} \\in \\mathbb{R}^{p+1}$, can be written as $\\c{1}{\\vect{x}} \\mapsto \\c{2}{f\\subtext{p}(\\vect{x};\\vect{w},a)}$, or simply $\\c{1}{\\vect{x}} \\mapsto \\c{2}{f\\subtext{p}(\\vect{x};\\vect{w})}$, with predefined $a$
]
.c20.center[
.diagram.center[
...
.compact[
$m$ layers, $l$-th layer of size $n\\sub{l}$, including the last one: .note[assume predefined $a$; $\\oplus$ is the concatenation of vectors]
