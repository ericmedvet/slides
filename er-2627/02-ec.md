...
- not simply a $s^\\star=\\argmax_\{s \in S\}$, because $\\prec$ is just a partial order! .note[$\\argmax$ can be used also to produce a set]
...
Main conferences:
- [EvoStar](https://www.evostar.org/), since 1998, Core **B**
  - in 2025 **held in Trieste**!
- [The Genetic and Evolutionary Computation Conference](https://gecco-2026.sigevo.org/HomePage) (GECCO), since 1999, Core **A** 
- [Parallel Problem Solving from Nature](https://ppsn2026.disi.unitn.it/) (PPSN), since 1989, Core **A**
- [IEEE Congress on Evolutionary Computation](https://attend.ieee.org/wcci-2026/) (CEC), since 1994, now part of IEEE World Congress on Computational Intelligence, Core **B**
- [Foundations of Genetic Algorithm](https://foga2027.dei.uc.pt/) (FOGA), since 1990, held evry two years, Core **A**

Main journals:
- [Genetic Programming and Evolvable Machines](https://link.springer.com/journal/10710), Springer, SJR **Q3**
...
6. a reproduction **selection criterion** .col2[$c\\subtext{repr}$]
  - given a bag $A$ and a partial order $\\prec$ on $A$, $c\\subtext{repr}(A,\\prec) \\in \\mathcal{P}\_A$ .note[i.e., $c\\subtext{repr}: \\mathcal{P}^*(A) \\times O\_A \\to \\mathcal{P}_A$, with $O\_A \\subseteq \\mathcal{F}\\sub{A \\times A \to \\\\{\\text{firts},\\dots\\\\}}$ being the set of all possible partial orders on $A$]
7. a $\\neg$survival **selection criterion** .col2[$c\\subtext{die}$]

1, 2, 3 depend on $S$; 6, 7 may depend on $S$; 4, 5 do not
]
.c50[
.pseudo-code.compact[
...
.footnote[
1. may be written as $S'' \\seteq \\min\\sub{\\prec} S'$
]
...
.compact[.ref[Romero, Philip A., and Frances H. Arnold. "Exploring protein fitness landscapes by directed evolution." Nature reviews Molecular cell biology 10.12 (2009): 866-876.]]
]
]
...
- "robots with 6 sensors on task $T$" $\\Rightarrow$ $B$, $o\\subtext{mut}$, $o\\subtext{xover}$ for 6-sensors robot
- "robots with 8 sensors on task $T$" $\\Rightarrow$  another $B$, $o\\subtext{mut}$, $o\\subtext{xover}$
- ...
...
$\\mathcal{L}(\\mathcal{G}) \\subseteq A\\sub{T}^\\ast$ is the **language** defined by $\\mathcal{G}$
...
**Input**:  
$\\vect{g}=\\texttt{01101001} \\; \\texttt{00001101} \\; \\texttt{01011000} \\; \\texttt{00000011} \\dots$  
...
| 6 | .bnf[(x1/(.col1[doe]))] | 48 | 11 | 5 | .bnf[4] |
| 7 | .bnf[(x1/(4.col1[oe]))] | 53 | 4 | 2 | .bnf[-] |
| 8 | .bnf[(x1/(4-.col1[e]))] | 16 | 3 | 2 | .bnf[.col1[v]] |
| 9 | .bnf[(x1/(4-.col1[v]))] | 203 | 3 | 3 | .bnf[x3] |
| 10 | .bnf[(x1/(4-x3))] | | | | &nbsp; |
...
.w75p.center[![Example of regression graphs](images/whge-mapping.png)]
...
2. properly present results
  - median of best fitness, or median $\\pm$ st. dev., or median with interquartile range vs. progress (.key[convergence plot])
...
**EA and representation**:
- GA with $n\\subtext{pop}=100$, $n\\subtext{tour}\\in \\{1, 5\\}$
...
- $\\phi = $ identity
$B=U([-1,1])^2$
- Gaussian mutation with $\\sigma\\subtext{mut}=0.35$
- segment crossover followed by Gaussian mutation

---

### In the search space ($\\mathbb{R}^2$) .note[color is $f\\sub{1}(\\vect{s})f\\sub{2}(\\vect{s})$]

.w80p.center[![Stages of the evolution in the search space with GA](exps/eat04-landscape-objs-ga.png)]
.w80p.center[![Stages of the evolution in the search space with ME](exps/eat04-landscape-objs-me.png)]
.w80p.center[![Stages of the evolution in the search space with NSGA-II](exps/eat04-landscape-objs-nsga2.png)]

.question[Question]: where are the optima?

---

### In the objectives space ($\\mathbb{R}^2$)

.w80p.center[![Stages of the evolution in the search space with GA](exps/eat04-objs-space-ga.png)]
.w80p.center[![Stages of the evolution in the search space with ME](exps/eat04-objs-space-me.png)]
.w80p.center[![Stages of the evolution in the search space with NSGA-II](exps/eat04-objs-space-nsga2.png)]

.question[Question]: where are the optima?

---

### Converge plots? .note[of one single run]

.cols[
.c20[
On $y$-axis, the $f\\sub{i}$ value for *one* best individual in the population
]
.c40[
.w100p.center[![Best fitness, 1st objective](exps/eat04-bestf-obj0-vs-evals.png)]
]
.c40[
.w100p.center[![Best fitness, 2st objective](exps/eat04-bestf-obj1-vs-evals.png)]
]
]
...
.cols[
.c20[
On $y$-axis, the lowest $f\\sub{i}$ value in the population
]
.c40[
.w100p.center[![Best fitness, 1st objective](exps/eat04-bestf-min-obj0-vs-evals.png)]
]
.c40[
.w100p.center[![Best fitness, 2st objective](exps/eat04-bestf-min-obj1-vs-evals.png)]
]
]

---

### All together: hypervolume

.cols[
.c60[
**Hypervolume**: the volume of the region of the objective space $\\mathbb{R}^p$ delimited by the undominated solutions in the population **and** a reference point (usually, the worst possible solution)
- a "measure" of the entire population
- the larger, the better
- not easily computable with $p>2$
]
.c40[
.w100p.center[![Hypervolume](exps/eat04-hv-vs-evals.png)]
]
]
