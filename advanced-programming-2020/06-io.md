...
array(35,190,20,20,5,5)
link([85,140,85,110,'>'])
array(160,30,20,20,9,5,8)
link([250,60,250,90,'>'])
array(165,150,20,20,9,8,8)
...
```java
is.read(buf, 0, 3);
// starts at t=0, ends at t~0, ret 3
is.read(buf, 0, 3);
...
2. Write an application that "uses" `FileArray`
  - receives $m>1$ parameters at command read
  - 1st is the name of the file
      - if not existing, create with $n \sim U(\\{1,\dots,2^5\\})$
      - othrewise, load
...
public class FileArray {
  ...
  public FileArray(String fileName, int n) { /* ... */ }
