...
- $S\_C=\\emptyset$ and $f\\suptext{out}\\sub{C} =$ RNN with predefined architecture (with enforced simmetry)
...
- **Behavioral fitness function** (BFF): there is an expected good behavior and the fitness captures it
  - **how is achieved** $\\to$ "high" a priori knowledge
...
2. evolved robot is used to collect pairs of $(\\vect{o}, \\vect{a}) \\in O \\times A = \\mathbb{R}\\sup{p\\subtext{in}} \\times \mathbb{R}\\sup{p\\subtext{out}}$
