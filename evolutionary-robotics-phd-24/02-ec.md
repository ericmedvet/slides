...
- $B$ a probability distribution over $S$ .note[$B$ for builder]
  - $B \\in \\mathcal{P}\_S$, i.e., $B: S \to [0,1]$, with $\\sum\_{s \\in S} B(s) = 1$
  - $s \\setsample B$ denotes sampling from $B$, briefly $\\sim B$
- a stochastic (unary) operator $o\\subtext{mut}$ on $S$ (**mutation**)
  - $o\\subtext{mut}: S \\to \\mathcal{P}\_S$, $s' \\setsample o\\subtext{mut}(s)$
- a stochastic binary operator $o\\subtext{xover}$ on $S$ (**crossover** $\to$ xover)
  - $o\\subtext{xover}: S \\times S \\to \\mathcal{P}\_S$, $s' \\setsample o\\subtext{mut}(s\_1,s\_2)$
- a population size $n\\subtext{pop} \\in \\mathbb{N}$
- a number of iterations $n\\subtext{gen} \\in \\mathbb{N}$
- a reproduction **selection criterion** $c\\subtext{repr}$
  - given a bag $A$ and a partial order $\\prec$ on $A$, $c\\subtext{repr}(A,\\prec) \\in \\mathcal{P}\_A$
- a reproduction **selection criterion** $c\\subtext{surv}$
]
.c50[
.pseudo-code.compact[
function $\\text{solve}(S, \\prec; B, o\\subtext{mut}, o\\subtext{xover}, n\\subtext{pop})$ {  
...
.i[]}  
.i[]$n \\seteq 0$  
.i[]while $n < n\\subtext{gen}$ { .comment[// iterations]  
.i[].i[]$n \\seteq n+1$  
