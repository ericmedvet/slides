...
.i[].i[].i[]$S\\subtext{pop} \\seteq S\\subtext{pop} \\oplus \\bag{s'}$  
.i[].i[]}  
...
.i[].i[].i[]$S\\subtext{pop} \\seteq S\\subtext{pop} \\c{3}{\\oplus \\bag{s'}}$  
.i[].i[]}  
.i[].i[]$S\\subtext{pop} \\seteq S\\subtext{pop} \\oplus S\\subtext{offspring}$  
...
**Efficiency**:  
👍 like $\\text{fitProp}$ with some sorting
...
**Example**:
- $S=\\{\\text{wing geometry for an airplane}\\}$ .note[$S$ may actually be $\\mathbb{R}^p$ and each $s$ a tuple of design parameters]
...
- $s_1 < s\_2$ if $f\\sub{j\_1}(s\_1)< f\\sub{j\_1}(s\_2)$ or $f\\sub{j\_1}(s\_1)=f\\sub{j\_1}(s\_2) \\land f\\sub{j\_2}(s\_1)< f\\sub{j\_2}(s\_2)$ or ...
  - compare 1st objective, if tie, then 2nd, if tie, then 3rd...
...
- sequences of symbols: .hspace2[] $\\to$ **discrete optimization**
