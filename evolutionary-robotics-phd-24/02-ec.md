...
**Linear GP (LGP)**
- a program $s$ is a sequence of tuples $((i\_j,r\\sub{\\text{in1},j},r\\sub{\\text{in2},j},r\\sub{\\text{our},j}))\_j$ (sort of machine language)
  - $i\_j \\in I$ is the instruction
  - $r\\sub{\\text{in1},j},r\\sub{\\text{in2},j},r\\sub{\\text{our},j} \\in \\{1,\\dots,n\\subtext{reg}\\}$ are the indices of input and output registers
...
.i[].i[]$n \\seteq n+n\\subtext{pop}$  
.i[].i[]$I\\subtext{pop} \\seteq \\text{sort}(I\\subtext{pop})$  
.i[].i[].col1[$\\vect{\\mu} \\seteq \\text{subseq}(I\\subtext{pop}, 1, \\lfloor r\\subtext{pop} n\\subtext{pop}\\rfloor)$]  
...
- $f(\\vect{s})=e^{-\\lVert\\vect{s}-\\vect{s}\_1^\\star\\rVert^2}+e^{-\\lVert\\vect{s}-\\vect{s}\_2^\\star\\rVert^2}$ 
  - one global optima at $\\vect{s}\_1^\\star=(2,2)$ and $\\vect{s}\_2^\\star=(-2,-2)$

**EA and representation**:
- GA with $n\\subtext{pop}=100$, $n\\subtext{tour}\\in \\{1, 2\\}$
- ES with $n\\subtext{pop}=30$, $\\sigma\\subtext{mut}=0.35$
]
.c20.center[
GA $n\\subtext{tour}=1$
